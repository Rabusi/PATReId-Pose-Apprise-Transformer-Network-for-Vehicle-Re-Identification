...
GPU_LIST = [0]
VALIDATION_RESULT_FILE = "reidentification/VeRi-Wild/Validation.txt"
...
def make_dataloader(cfg):
    ...
    return train_loader, train_loader_normal, val_loader_3000, len(dataset.query_3000), num_classes, cam_num, view_num
...
def do_train(cfg,
    ...
    for epoch in range(begin_epoch, epochs + 1):
        ...
        model.train()
        for n_iter, (img, vid, target_cam,target_color,target_type) in enumerate(train_loader):
            img = img.cuda()
            target = vid.cuda()
            target_cam = target_cam.cuda()
            target_color=target_color.cuda()
            target_type=target_type.cuda()
            score,score_color,score_type, feat = model(img, target, cam_label=target_cam )
            loss = loss_fn(score,score_color,score_type, feat, target, target_cam,target_color,target_type)
            ...
            if ((n_iter + 1) % accum_iter == 0) or (n_iter + 1 == len(train_loader)):
              optimizer.step()
              optimizer.zero_grad()
              del img,target,target_cam,target_color,target_type,score,score_color,score_type, feat,loss
              torch.cuda.empty_cache()


            torch.cuda.synchronize()
            if (n_iter + 1) % log_period == 0:
                print("Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Acc: {:.3f}"
                            .format(epoch, (n_iter + 1), len(train_loader),
                                    (loss_meter.avg)*accum_iter, acc_meter.avg))
        scheduler.step()
        end_time = time.time()
        ...
        train_loss[epoch]=loss_meter.avg
        states={
        ...
        if epoch>=40 and epoch % checkpoint_period == 0:    
          torch.save(states,
                    os.path.join(cfg.OUTPUT_DIR, 'checkpoint'+'_{}.pth'.format(epoch)))
        ...
        if epoch % eval_period == 0:
          ...
          plt.ylabel("CMC(k)")
          plt.title("CMC Curve")    
          plt.savefig(os.path.join(cfg.OUTPUT_DIR,"epoch_{}.png".format(epoch)))
          plt.close()   
          for r in[1, 5, 10]:
...
def do_inference(cfg,
    ...
    for n_iter, (img, pid, camid, camids,_,_, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            img = img.to(device)
            camids = camids.to(device)
            feat = model(img, cam_label=camids)
    ...
    plt.ylabel("CMC(k)")
    plt.title("CMC Curve")  
    plt.savefig(os.path.join(cfg.OUTPUT_DIR,"inference.png"))
    plt.show() 
    plt.close()  
    for r in [1, 5, 10]:
...
optimizer = torch.optim.SGD(model.parameters(),lr=0.01,momentum=cfg.SOLVER.MOMENTUM)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfg.SOLVER.BASE_LR, eta_min=0.01 * cfg.SOLVER.BASE_LR)
